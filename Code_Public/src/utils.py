...
def cluster_vector_distance_variance_threshold(cluster, center1, center2, multiplier):
    ...
    #dump(distances, '/home/jan/CAU/Masterarbeit/master-thesis-jan-bensien/Code/Plots/distances_ccv_' + str(cluster.values[0]) +'.pkl')
    m = np.mean(distances)
...
def line_between_two_points(point1, point2):
    #point1 = np.array([1, 2, 3, 4]) #any numbers, be it 4, 5 or n dimensions
    #point2 = np.array([3, 2, 4, 5])
...
def calculate_thresholds_basic(data, clf, multiplier):
    ...
    # Workaround for testing - calculate distance from center and set highest as threshold
    # cluster_0 = data[data['cluster'] == 0].drop(columns=['cluster'])
    #center_cluster_0 = cluster_0.mean(axis = 0)
    #cluster_1 = data[data['cluster'] == 1].drop(columns=['cluster'])
    #center_cluster_1 = cluster_1.mean(axis = 0)
    #max_distance_cluster_0 = cluster_0.apply(lambda row: distance.euclidean(row, center_cluster_0), axis=1).max()
    #max_distance_cluster_1 = cluster_1.apply(lambda row: distance.euclidean(row, center_cluster_1), axis=1).max()
    #t2 = max(max_distance_cluster_0, max_distance_cluster_1)
...
def calculate_thresholds_gaussian(data, clf, multiplier):
    ...
    # To visualize the distribution save the distances
    #dump(distances_cluster_0, '/home/jan/CAU/Masterarbeit/master-thesis-jan-bensien/Code/Plots/distances_cluster_0.pkl')
    #dump(distances_cluster_1, '/home/jan/CAU/Masterarbeit/master-thesis-jan-bensien/Code/Plots/distances_cluster_1.pkl')
